...
public class AutoMethod {
  ...
  private final PIDController m_xController = new PIDController(0.9, 0, 0);
  private final PIDController m_yController = new PIDController(0.9, 0, 0);
  private final PIDController m_thetaController = new PIDController(1.1, 0, 0);
  ...
  public AutoMethod(DriveTrainSubsystem driveTrainSubsystem, Dashboard dashboard, ShooterSubsystem shooter, IntakeSubsystem intake, TransitionSubsystem transition, ShooterAngleSubsystem shooterAngle, Limelight limelight) {
    ...
    m_autoFactory = new AutoFactory(
        ...
        m_driveTrainSubsystem::swerveOnlyResetPose,
        this::followSample,
        true,
  ...
  public Command getAutonomousCommand() {
      ...
      private void followSample(SwerveSample sample) {
        ...
        speeds = speeds.plus(new ChassisSpeeds(
            m_xController.calculate(currentPose.getX(), sample.x),
            m_yController.calculate(currentPose.getY(), sample.y),
            m_thetaController.calculate(currentPose.getRotation().getRadians(), sample.heading)));
        m_driveTrainSubsystem.driveRobotRelative(speeds);
      }

      public Command makeSwerveAutoCommand(String pathString) {
        return m_autoFactory.trajectoryCmd(pathString);
      }

      public void resetPose(String pathString) {
        boolean isRed = Robot.getAllianceColor() == Alliance.Red;
        Choreo.<SwerveSample>loadTrajectory(pathString).ifPresent(path -> {
          path.getInitialPose(isRed).ifPresent(pose -> {
            double initRotation = pose.getRotation().getRadians() + Math.toRadians(getGyroResetAngle());
